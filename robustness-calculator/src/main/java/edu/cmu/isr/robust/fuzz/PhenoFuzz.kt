...
class PhenoFuzz(val env: String) {
  ...
  fun mutationIter(trace: List<String>): Iterator<Pair<String, List<String>>> {
    return mutationIterator(trace,MAX_ACTIONS,phenotypes,alphabet)
  }
...
private class TraceIterator(val sm: StateMachine, val K: Int): Iterator<List<String>> {
  ...
  private class mutationIterator(trace:List<String>, MAXACTION:Int, phenotypes:List<Pair<String, List<String>>>, alphabet: List<String>): Iterator<Pair<String, List<String>>> {
    private val trace = trace
    private val MAX_ACTIONS = MAXACTION
    private val phenotypes = phenotypes
    ...
    override fun hasNext(): Boolean {
      //TODO: limit number? check for repeats?
      if(count < max)
        return true
      else {
        return false
    ...
    override fun next(): Pair<String, List<String>> {
      count++
      var errorTrace = generateErrorTrace()
      var phenotype = errorTrace.first
      var trace = errorTrace.second
      while (trace == null) {
        errorTrace = generateErrorTrace()
        phenotype = errorTrace.first
        trace = errorTrace.second
      }
      return Pair(phenotype, trace)
    }

    private fun generateErrorTrace(): Pair<String, List<String>?> {
      ...
      return Pair(error.first,errorTrace)
    }
    /**
      ...
      for (rule in error.second) {
        ...
        if (insertionORrule.contains(Regex("^i_.*"))) {
          ...
          if (toAdd!=null) {
            ...
            errorEncodedString.add("error_omission_${error.first}${sb.toString()}")
          }
